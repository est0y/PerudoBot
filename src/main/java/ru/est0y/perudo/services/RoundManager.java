...
public class RoundManager {
    ...
    public void finishRound(JDA jda, Game game) {
        ...
        //todo исправить
        var noBeliever = game.getPlayers().stream()
                .filter(player -> player.getNumber() == game.getTurnHolder().getNumber()).findFirst().orElseThrow();
        ...
        game.getPlayers().stream().filter(p -> !p.equals(loser))
                .forEach(p -> playerUtils.rollDice(p, p.getDice().size()));
        if (game.isSpecialRound()) {
        ...
        if (newLoserDiceCount == 0) {
            var playersHaveMoreOneDice = game.getPlayers().stream().filter(p -> !p.getDice().isEmpty()).toList();
